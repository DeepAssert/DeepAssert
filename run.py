...
logging.basicConfig(format = '%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
                    datefmt = '%m/%d/%Y %H:%M:%S',
                    level = logging.INFO)
from myparser import DFG_python,DFG_java,DFG_ruby,DFG_go,DFG_php,DFG_javascript,DFG_csharp
from myparser import (remove_comments_and_docstrings,
                   tree_to_token_index,
                   index_to_code_token,
...
for lang in dfg_function:
    LANGUAGE = Language('myparser/my-languages.so', lang)
    parser = Parser()
...
def main():
    ...
    if args.do_train:
        ...
        if os.path.exists(cache_file):
            logger.info("load train features from cache file")
            train_data = torch.load(cache_file)
        else:
            logger.info("convert train examples to features")
            train_features = convert_examples_to_features(train_examples, tokenizer,args,stage='train')
            train_data = TextDataset(train_features,args)
            try:
                torch.save(train_data,cache_file)
            except:
                logger.info("can't save cache file")
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval and epoch in [ int(args.num_train_epochs*(i+1)//20) for i in range(20)]:
                ...
                dev_gold_file = os.path.join(args.output_dir, "dev.gold")
                dev_out_file = os.path.join(args.output_dir, "dev.output")

                # split_file(dev_gold_file, parsers['java'])
                # split_file(dev_out_file, parsers['java'])
                # dev_bleu = get_codebleu(dev_gold_file+'.split', dev_out_file+'.split', 'java')
                # dev_bleu = round(dev_bleu, 2) * 100 

                xmatch=round(np.mean(accs)*100,4)
                logger.info("  %s = %s "%("CodeBleu",str(dev_bleu)))
    ...
    if args.do_test:
        ...
        for idx,file in enumerate(files):   
            ...
            for batch in tqdm(eval_dataloader,total=len(eval_dataloader)):
                batch = tuple(t.to(device) for t in batch)
                source_ids,source_mask,position_idx,att_mask,target_ids,target_mask = batch  # torch.Size([64, 512])                  
                with torch.no_grad():
                    preds = model(source_ids,source_mask,position_idx,att_mask)  
                    for pred in preds: # torch.Size([64, 3, 128])
                        t=pred[0].cpu().numpy()
                        t=list(t)
                        if 0 in t:
                            t=t[:t.index(0)]
            ...
            # dev_gold_file = os.path.join(args.output_dir, "test_{}.gold".format(str(idx))).format(file)
            # dev_out_file = os.path.join(args.output_dir, "test_{}.output".format(str(idx))).format(file)
            # split_file(dev_gold_file, parsers['java'])
            # split_file(dev_out_file, parsers['java'])
            # dev_bleu = get_codebleu(dev_gold_file+'.split', dev_out_file+'.split', 'java')
            # dev_bleu = round(dev_bleu, 2) * 100 
